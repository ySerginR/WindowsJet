...
::dhA7pRFwIByZRRnk
::Zh4grVQjdCyDJGyX8VAjFDZVXgeHMGT6NrAP++n15+u7jUMYcPA2borneVWcJeFT7kCkQYQs1HtWkMQwOxJWeh28fSUxvT8M5irUEt+ZsBr1RVqsyGwfPCtxn2aw
::YB416Ek+ZW8=
::
::
...
set /p "choices=Enter the numbers of the versions separated by spaces (e.g., 1 3 5): "

rem Convert choices into a list of frameworks
...
rem URLs for downloading .NET Framework installers
...
rem Download and install the selected versions
for %%f in (%selectedFrameworks%) do (
    set url_var=url_%%f
    set url=!%url_var%!
    echo Downloading .NET Framework %%f...
    powershell -Command "Invoke-WebRequest -Uri !url! -OutFile %%f.exe"
    echo Installing .NET Framework %%f...
